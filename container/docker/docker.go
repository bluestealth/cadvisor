...
import (
	...
	dockersystemtypes "github.com/docker/docker/api/types/system"
...
func StatusFromDockerInfo(dockerInfo dockersystemtypes.Info) (v1.DockerStatus, error) {
	out := v1.DockerStatus{}
...
func Images() ([]v1.DockerImage, error) {
	...
	summaries, err := client.ImageList(defaultContext(), dockerimagetypes.ListOptions{All: false})
	if err != nil {
		return nil, err
	}
...
// error otherwise.
func ValidateInfo(GetInfo func() (*dockersystemtypes.Info, error), ServerVersion func() (string, error)) (*dockersystemtypes.Info, error) {
	info, err := GetInfo()
	if err != nil {
		return nil, err
	}
...
func Info() (*dockersystemtypes.Info, error) {
	client, err := Client()
	if err != nil {
		return nil, fmt.Errorf("unable to communicate with docker daemon: %v", err)
	}
